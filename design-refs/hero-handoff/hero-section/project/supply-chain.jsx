...
// The wheel sits low so icons ride the top arc like the reference.
const WHEEL = {
  cx: STAGE_W / 2,
  cy: 760, // center sits below the canvas
  rOuter: 540,
  rInner: 380,
};
...
// ── Background: deep purple sky with a soft sunrise glow ────────────────────
function SkyBackground() {
  const t = useTime();
  // Subtle parallax: glow drifts very slowly
  const glowX = STAGE_W / 2 + Math.sin(t * 0.3) * 20;
  return (
    <>
      <div style={{
        position: 'absolute', inset: 0,
        background: `radial-gradient(ellipse 90% 70% at ${glowX}px 100%,
          ${C.berryPurple} 0%,
          ${C.deepPurple} 45%,
          ${C.darkIndigo} 100%)`,
      }} />
...
function QRCodeBadge() {
  ...
  // Sit the QR badge in the upper part of the cream interior
  const badgeW = 240;
  const badgeH = 320;
  const badgeX = WHEEL.cx - badgeW / 2;
  const badgeY = WHEEL.cy - WHEEL.rInner + 30;
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        borderRadius: 999,
        whiteSpace: 'nowrap',
        boxShadow: '0 4px 12px rgba(59,3,102,0.4)',
        letterSpacing: '0.02em',
      }}>
        {label}
        {sublabel && (
          <span style={{ fontWeight: 500, opacity: 0.7, marginLeft: 6, fontSize: 12 }}>
            · {sublabel}
          </span>
        )}
      </div>
...
function Title() {
  ...
  // Intro eyebrow & subtitle: 0.2 → 1.5s
  // Final tagline: 4.6 → 6.0s
  return (
    <>
      {/* Intro eyebrow */}
      <Sprite start={0.2} end={1.6}>
        <div style={{
          position: 'absolute',
          left: 80, top: 80,
          fontFamily: 'JetBrains Mono, monospace',
      ...
      <Sprite start={0.4} end={1.8}>
        <div style={{
          ...
          <FadeSlide delay={0.1}>
            From crop<br/>
            <span style={{ color: C.vibrantYellow, fontStyle: 'normal' }}>to craft.</span>
          </FadeSlide>
        </div>
      </Sprite>
  ...
  return (
    <div data-screen-label="t=0s" style={{ position: 'absolute', inset: 0 }}>
      <Stage
        ...
        height={STAGE_H}
        duration={DUR}
        background={C.darkIndigo}
        loop={true}
        autoplay={true}
        persistKey="cocoa-supply-chain"
      >
        <Scene />
      </Stage>
    </div>
